...
ready(function () {
	...
	if (window.Worker) {
		var number_of_simulations_input;
		var worker;

		(function () {
			var initializeWorker = function initializeWorker() {
				worker = new Worker('./js/worker.es5.js');
			...
			// Style numeral input
			number_of_simulations_input = new Cleave('#simulations', {
				numeral: true,
		...
		$1('#main').innerHTML = 'Sorry, your web browser must support ' + '<a href="https://developer.mozilla.org/en-US/docs/Web/API/Web_Workers_API">Web Workers</a> ' + 'in order to run this page.';
	}
});